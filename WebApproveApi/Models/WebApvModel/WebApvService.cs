...
namespace WebApproveApi.Models.WebApvModel
{
    public class WebApvService
    {
        ...
        // เช็ค cancel กรณีไม่สามารถเข้าสู้ระบบได้อีก
        #region validateApproveCancel
        public DocumentDetail validateApproveCancel(string numOfDocument, int regId)
        {
            try
            {
                return conn.DocumentDetails
                    .Where(a => a.numOfDocument == numOfDocument && a.regId == regId)
                    .OrderByDescending(b => b.dateRequest).FirstOrDefault();
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public List<sp_PullingService_Result> PullingService()
        {
            try
            {
                return conn.sp_PullingService().ToList();
            }
            catch(Exception ex)
            {
                throw ex;
