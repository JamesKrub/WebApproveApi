...
namespace WebApproveApi.Controllers
{
    public class ValidateSchemaController : ApiController
    {
        ...
        public string RequestDocument([FromBody]RequestData value)
        {
            ...
            //var xml = "<PurchaseOrders xmlns=\"PurchaseForm\"><PurchaseOrder><purchaseId>2</purchaseId><Header>ไปเที่ยว</Header><Name>385</Name><Date>2002-05-30T09:00:00</Date><Price>500.50</Price><NumberQty>5</NumberQty></PurchaseOrder></PurchaseOrders>";
            var xml = value.dataRequest;

            //var user = XElement.Parse(xml);
            //string numOfDocument = user.Value.FirstOrDefault().ToString(); //เก็บ primary key ของเอกสาร ยังเก็บแค่ค่าแรก อักษรตัวแรก!!!
            List<readFromXML> dataXML = new List<readFromXML>();
            System.Xml.XmlDocument doc = new System.Xml.XmlDocument();
            doc.LoadXml(xml); // โหลด string
            foreach (System.Xml.XmlElement document in doc.DocumentElement.ChildNodes)
            {
                ...
                //    string numOfDocument = field.InnerText.FirstOrDefault().ToString();

                //}
                value.numOfDocument = document.ChildNodes[0].InnerText;
            }
            ...
            try
            {
                ...
                {
                    DocumentDetail Data = service.validateApproveCancel(value.numOfDocument, value.regId);

                    if (Data == null || (Data.statusId == "03") )
                    {
                        value.regId = data.regId;
                        //value.numOfDocument = numOfDocument;
